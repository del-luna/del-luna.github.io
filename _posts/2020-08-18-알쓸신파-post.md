...
```python
#키워드 파라미터에 값을 할당할 때는 띄어쓰기를 사용하지 않는다.
...
이렇게 하면 a=1,   a = 1 로 검색할 때 서로 다른 부분을 찾기 편함

### Naming

- 변수 혹은 함수의 리턴이 시퀀스일 경우 s를 붙인다.

```python
def calc_price(a=name):
	return 100

def calc_prices(a=name):
	return [100,200,300]
```

- prices 대신 price_list도 좋은 네이밍이 될 수 있지만(심지어 이건 타입이 list인 경우 타입 힌트까지 보여줄 수 있음) 모든 변수마다 _dict, _tuples를 할 수 없으므로 prices처럼 쓰는게 나을 것같음.

### Type annotation

- 아예 리턴타입을 명시해줄  수 있기도함.

```python
def calc_price_by_name(name: str) -> int:
	"""하나의 입력 하나의 출력"""
	pass

def calc_price_by_names(name: list) -> int:
	"""리스트를 입력으로 받고 하나를 출력"
	pass
```

- 타입 어노테이션까진.. 굳이 할 필요 있을까 싶긴 함
- 근데 이렇게까지하면 docstring을 굳이 안써도 네이밍으로 인풋 아웃풋에 대한 예측이 가능함.

### 반복문의 변수는 무조건 간략하게

```python
#이거보단
drinks = [product for product in procuts]
#이게 낫다
drinks = [p for p in products]
```

### 클래스 내부에서만 사용하는 속성, 메서드는 _를 붙인다.

- 파이썬은 private, public등 접근 제어자가 없지만 위 같은 컨벤션을 통해서 사용할 수 있다.(실제로 private이 되는게 아니라 그냥 약속)
...
### get은 클래스 외부에서 가져올 때, find는 주어진 파라미터에서 찾을 때
