...
cfg가 커지면 따로 .cfg 파일로 빼서 사용한다고 합니다.(물론 이 경우 따로 파싱하는 코드가 필요합니다.)
...
파이토치의 trasform을 통한 augmentation은 데이터셋의 길이가 늘어나진 않습니다.

따라서 Random으로 붙은 녀석들(e.g. RandomRotation)은 어차피 에폭을 도는동안 기존의 이미지 그대로 나올수도, 변환이 되어서 나올 수도 있으므로 굳이 데이터셋의 크기를 늘려서(기존+ 변환된 이미지) 학습할 필요는 없습니다.

저는 이게 처음엔 너무 헷갈려서 아니 왜 변환했는데 데이터셋의 크기가 안늘어나지??? 라고 생각했었습니다. 사실 그럴 필요가 없는거죠..!
...
많은 사람들이 albumentation이라는 라이브러리를 주로 사용합니다.
...
```Python
class DropPixel(object):
    
    def __call__(self, pic):
        return self.drop(pic)
    ...
    def drop(self, pic):
        if not isinstance(pic, Image.Image):
            raise TypeError("img should be PIL Image. Got {}".format(type(pic)))

        origin = np.array(pic)
        max_pixel = np.where(origin>10, origin, 0)
        if random.random()>=0.5:
             return Image.fromarray(origin, mode='L')
        else:
             return Image.fromarray(max_pixel, mode='L')
```

이런 식으로 커스텀 트랜스폼을 만들 수 있습니다.

리턴은 pillow타입이어야 다른 트랜스폼과 연결 가능하므로 저런식으로 해줬습니다.
...
Transforms 에서 rotation과 같은 이미지 변환시에 입력 이미지 타입이 ndarray면 발생하는 문제입니다. Pillow이미지 타입으로 받으면 해결됩니다.
