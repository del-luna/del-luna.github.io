...
왼쪽은 하나의 item rating vector$(R^T)_i \in R^{1*N}$을 하나의 샘플(i.e. one column of R)로 입력하고, 오른쪽은 하나의 user rating vector$R_u \in RY{1*M}$을 하나의 샘플(i.e. one row of R)로 입력합니다.
