...
BN처럼 활성화의 first-order moment를 수정한 다음 $p=1,2,...,\infty$에 대해 $l_p-norm$(평균 이동 전)의 평균으로 정규화 하는 방식을 연구합니다. 이런 정규화 체계의 경우 레이어의 입력 분포가 더 이상 가우스와 유사하지 않습니다. 따라서 이러한 $l_p-norm$을 사용한 정규화는 더 이상 분포 모멘트나 분포 안정성에 대한 제어를 보장하지 않습니다.

>  Specifically, we study schemes that fix the first order moment of the activations, as BatchNorm does, and then normalizes them by the average of their p-norm (before shifting the mean), for p = 1, 2, ∞. Note that for these normalization schemes, the distributions of layer inputs are no longer Gaussian-like (see Figure 14). Hence, normalization with such p-norm does not guarantee anymore any control over the distribution moments nor distributional stability
>
> 솔직히 뭔소린지 아직 잘 모르겠다.. BN이 왜 안전성에 대한 제어를 보장하며 (ICS)얘기인 것 같은데... $L_p-norm$계열은 왜 보장하지 않는다고 말할 수 있지?
>
> 가우스 분포랑 관련있는건가?
...
>
> 헤시안의 컴포넌트에 따라 더 볼록, 덜 볼록 이 되니까 이게 바운디드 되면 기울기가 좀 더 예측 가능해지지 않을까..?
>
> 아무튼 바운딩 되면 스텝을 좀 더 크게 줄 수 있음(그래디언트의 예측 가능성 증가, 즉 이 방향으로 가도 된다는 신뢰도 증가..?)



BN은 스케일링만 수행하는 것이 아닙니다. 결과적으로 normal landscape의 모든 최소값이 BN landscape에 보존됩니다.
